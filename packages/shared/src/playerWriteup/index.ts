...
/**
 * WHAT THIS ENGINE NEEDS OF A PLAYER, AND NOTHING MORE (2026-09-11).
 *
 * Until today the parameter type was `HockeyPlayer`, the prop type of a
 * React component in `apps/web`. That was survivable while the engine ran
 * only in the browser; it is not survivable now that the server renders the
 * same object, because dragging a component's prop type into
 * `@citrus/shared` would put a card's `nextGame.gameStatus` and `image` on
 * the server's conscience for no reason.
 *
 * So the contract is STRUCTURAL: exactly the fields the 800 lines below
 * actually read, and every one of them optional except the two that name
 * the player. `HockeyPlayer` satisfies it (the web call site asserts so),
 * and so does a row the server assembles out of `player_directory` and
 * `player_season_stats` without either side knowing about the other.
 *
 * Each stat below is the same NHL.com counting stat the card prints beside
 * the prose, with one exception: `xGoals` is `player_season_stats.x_goals`,
...
function fullName(player: WriteupPlayer): string {
 ...
 */
function seedOf(player: WriteupPlayer): number {
...
function buildGoalieWriteup(player: WriteupPlayer): PlayerWriteup {
...
function buildSkaterWriteup(player: WriteupPlayer): PlayerWriteup {
  const s = player.stats || {};
  const gp = s.gamesPlayed ?? 0;
  const name = firstName(player.name);
  const tags: WriteupTag[] = [];
