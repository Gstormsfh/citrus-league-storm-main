...
import { BrowserDraftKitService } from '../services/BrowserDraftKitService';
...
draftKitExternalRoutes.get('/capabilities',c=>ok(c,{yahoo:enabled('yahoo'),espn:enabled('espn'),browser:process.env.DRAFT_KIT_BROWSER_COMPANION_ENABLED==='true',pollAfterMs:15000}));
draftKitExternalRoutes.post('/browser-kit',async c=>{
  try {
    if(process.env.DRAFT_KIT_BROWSER_COMPANION_ENABLED!=='true')throw AppError.serviceUnavailable('The browser companion is still being verified. No new purchase is needed.');
    const parsed=z.object({platform:z.enum(['espn','yahoo']),league:z.string().trim().min(1).max(64),weights:z.record(z.string(),z.record(z.string(),z.number().finite().min(-10000).max(10000)))}).strict().safeParse(await c.req.json().catch(()=>null));
    if(!parsed.success)throw AppError.badRequest('Check the companion provider and kit settings.');
    const db=createUserClient(c.get('userToken')),userId=c.get('userId'),access=new DraftKitAccessService(db);
    if(!(await access.access(userId)).active)throw AppError.forbidden('An active draft kit is required.');
    const result=await new BrowserDraftKitService(db,supabaseAdmin).open(parsed.data.platform,parsed.data.league,parsed.data.weights);
    if(!(await access.access(userId)).active)throw AppError.forbidden('Your draft-kit access has changed.');
    await new AuditService(db).log('DATA_EXPORT',null,{action:'browser_companion_open',platform:parsed.data.platform});
    return ok(c,result);
  }catch(error){return handleError(c,error instanceof AppError?error:AppError.badGateway('Could not prepare your companion. Your existing draft has not changed.'),'Could not prepare browser companion');}
});
