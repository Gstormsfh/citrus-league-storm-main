...
describe('Draft Kit website checkout readiness', () => {
  ...
  it('fulfils a completed session instead of creating a second payable checkout before webhook delivery', async () => {
    const userDb = database(); userDb.rpc.mockRejectedValue(new Error('browser client must never fulfil'));
    const adminDb = database();
    adminDb.rpc.mockResolvedValueOnce({ data: [{ attempt_id: '22222222-2222-4222-8222-222222222222', checkout_session_id: 'cs_completed' }], error: null })
    ...
    await expect(new DraftKitCheckoutService(userDb as any, config, provider as any, adminDb as any)
    ...
    expect(adminDb.rpc).toHaveBeenCalledWith('fulfill_draft_kit_checkout', expect.objectContaining({ p_session: 'cs_test_kit' }));
  });
  it('refuses fulfilment when Stripe reports the payment refunded before webhook delivery', async () => {
    const db = database(); const refunded = paidSession();
    refunded.payment_intent = { ...refunded.payment_intent, latest_charge: { refunded: true, disputed: false } } as any;
    const provider = { webhooks: { constructEvent: vi.fn().mockReturnValue({ livemode: false, type: 'checkout.session.completed', data: { object: { id: 'cs_test_kit' } } }) }, checkout: { sessions: { retrieve: vi.fn().mockResolvedValue(refunded) } } };
    await expect(new DraftKitCheckoutService(db as any, config, provider as any).webhook('{}', 'sig')).rejects.toMatchObject({ status: 400 });
    expect(db.rpc).not.toHaveBeenCalledWith('fulfill_draft_kit_checkout', expect.anything());
  });
