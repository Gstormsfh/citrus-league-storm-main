...
export class PlayerService {
  ...
  async getAllPlayers() {
    ...
    const talentMap = new Map((talents || []).map((t: any) => [t.player_id, t]));
    const gsaxMap = new Map((gsax || []).map((g: any) => [g.player_id, g]));

    const players = (directory || []).map((p: any) => {
    ...
    players.sort((a: any, b: any) => (b.points || 0) - (a.points || 0));
  ...
  async getPlayersByIds(playerIds: (string | number)[]) {
    if (!playerIds.length) return { players: [], error: null };
    ...
    const dirIds = (directory || []).map((p: any) => p.player_id);
  ...
  async searchPlayers(query: string) {
    ...
    return {
      players: players.filter((p: any) => p.full_name?.toLowerCase().includes(q)),
      error: null,
    };
  }
