-- Revert populate_player_weekly_stats to use PBP stats (not NHL stats)
-- Reason: We only populated season totals, not per-game NHL stats
-- Per-game NHL stats are a future enhancement
-- For now, aggregate PBP stats for matchup weeks (accurate enough)
...
AS $$
BEGIN
  -- Insert or update weekly stats using PBP-calculated stats from player_game_stats
  -- NHL stats columns exist but will remain 0 until per-game NHL stats are populated
  INSERT INTO public.player_weekly_stats (
    player_id,
    week_number,
    week_start_date,
    week_end_date,
    -- PBP-calculated stats (used for matchup weeks)
    goals,
    primary_assists,
    secondary_assists,
    shots_on_goal,
    ...
    shots_faced,
    shutouts,
    -- NHL stats columns (will be 0 until per-game NHL stats are populated)
    nhl_goals,
    nhl_assists,
    nhl_points,
    nhl_shots_on_goal,
  ...
  SELECT
    ...
    p_week_start_date,
    p_week_end_date,
    -- PBP-calculated stats (for matchup weeks)
    ...
    -- NHL stats (will be 0 until per-game NHL stats are populated - future enhancement)
  ...
  DO UPDATE SET
    -- Update PBP stats (used for matchup weeks)
    ...
    -- Update NHL stats (will be 0 until per-game NHL stats are populated)
