-- Auto-generate playoff brackets when the regular season completes.
--
-- Changes:
-- 1. generate_playoff_bracket now allows callers with auth.uid() IS NULL
--    (service role / backend) to bypass the commissioner check, and also
--    restricts bracket generation to fantasy leagues.
-- 2. New function auto_generate_playoff_bracket(p_league_id) checks all
--    conditions (fantasy league, draft completed, no existing bracket,
--    all regular-season matchups completed) and generates the bracket
--    using the league's stored options.
--
-- The server calls auto_generate_playoff_bracket() on every getBracket
-- request via the service-role client, so every league automatically gets
-- a bracket the moment its regular season ends — no commissioner action.
--
-- Leagues can opt out by setting settings.autoPlayoffs = false.
