-- =============================================================================
-- N-2 / KI-034 — v2 draft-completion clears leagues.draft_state
-- =============================================================================
--
-- Authored 2026-08-08. AUTHORED-NOT-APPLIED per unattended-day directive
-- (Garrett executes on return). Applies AFTER F27 chunk (start_draft_v2)
-- has landed — the two together make draft_state's lifecycle honest.
--
-- MOTIVATION (N-2 architect finding, worklog 2026-08-08):
-- STEP 5' green run on c3615619 (2026-08-08T06:38Z) observed
-- `leagues.draft_state='active'` post-completion, alongside
-- `draft_status='completed'` + `pick_deadline=NULL`. The column WAS set
-- to 'active' by start_draft_v2's ignition path (migration
-- 20260807000000). This migration's F24 rebase (below) DELIBERATELY left
-- draft_state UNTOUCHED at completion under Amendment 2 (2026-08-05
-- ruling — that day's prod query returned ERROR:column does not exist
-- for v2 consumers). Post-F27 that Amendment is superseded: the column
-- exists, is populated at ignition, and reads inconsistently at
-- completion.
--
-- ARCHITECT DIRECTIVE 2026-08-08 chose Option (A) over Option (B):
--   (A) F24 completion path ALSO writes draft_state='completed'.
--       Small, keeps the column operationally accurate.
...
--     AFTER:  SET draft_status = 'completed', pick_deadline = NULL,
--                 draft_state  = 'completed'
--
-- The value 'completed' is admitted by `leagues_draft_state_chk`
-- (migration 20260425130000_draft_engine_v2_foundation.sql:140-142):
-- `CHECK (draft_state IN ('not_started','pre_draft','active','paused',
-- 'completed','cancelled'))`.
--
-- SUPERSEDES: 20260805050000_v2_draft_completion_emitter_rebased.sql
--   (which itself SUPERSEDED 20260805023419 per F25 rebase).
--
-- STANDING RULES applied (per docs/MIGRATION_SAFETY_GUIDE.md §368+):
--   Rule 1 — capture-before-replace: TODO Garrett-exec at apply time.
--     Capture file `supabase/migrations/captures/2026-08-08_pre_v2_draft_completion_clears_draft_state.sql`
--     must be populated with pg_get_functiondef(...) output on the
--     staging DB the same day the apply runs. Author-mode today: placeholder file created + apply-harness
--     STEP 0 hash-pins against it. See apply-harness for the mechanic.
--   Rule 2 — real SQL in direct-apply history rows: apply-harness uses
--     the psql `\lo_import` → `lo_get` → `convert_from(bytea, 'UTF8')`
--     round-trip so `supabase_migrations.schema_migrations.statements[1]`
--     is byte-exact.
--   Rule 3 — force `client_encoding=UTF8`: apply-harness sets it as
--     first statement + `SHOW client_encoding` echoes for operator.
--
-- LEGACY BODY BELOW is F24 rebase body verbatim except the ONE change
-- and the surrounding comment update. Everything else — Amendment 1/3/4
-- notes, D1-D8 rulings, event_version=1/2 discipline, structural SUM
-- pattern, mirror invariant on Amendment 3 — carries over.
-- =============================================================================
--
-- Historical header from the F24 rebase (preserved so future reviewers
-- can trace the chain — DO NOT delete when regenerating captures):
--
...
--
-- F25 (2026-08-05, first machine-found defect on the campaign ledger):
...
BEGIN
  ...
  IF v_total_picks > 0 AND p_pick_number >= v_total_picks THEN
    ...
    -- Final pick. Flip status + clear deadline + clear draft_state +
    ...
    --
    -- N-2 / KI-034 (2026-08-08): draft_state ALSO clears to 'completed'.
    -- Amendment 2 (2026-08-05 evidence-close) said the column was v2-
    -- stack-only and no v2 consumer read it — that ruling was
    -- superseded by F27 (start_draft_v2, migration 20260807000000)
    -- which now sets draft_state='active' at ignition. Without the
    -- matching completion clear, STEP 5' on c3615619 observed
    -- draft_state='active' + draft_status='completed' + pick_deadline=
    -- NULL — inconsistent read for any future consumer. This UPDATE
    -- restores column-consistency at completion. Value 'completed'
    -- is admitted by leagues_draft_state_chk (foundation migration
    -- 20260425130000:140-142). Legacy readers: v1's
    -- complete_draft_and_sync (migration 20260321000000) writes only
    -- draft_status; it does NOT gate on draft_state, so v1 behavior
    -- is unchanged.
    UPDATE public.leagues
...
COMMENT ON FUNCTION public.submit_pick_v2(uuid,uuid,int,int,int,uuid,uuid,text,jsonb,uuid) IS
  'Spec §4.5 / §5.2: the pick path. Chunk 10c-2 batch 2 (2026-07-27): pick event payload carries pick_deadline for engine external-apply timer re-arm; event_version=2. F24 (2026-08-05, rebased from stale base per F25): completion branch added — final pick flips draft_status=completed, clears leagues.pick_deadline (Amendment 1), emits draft_completed lifecycle event via append_draft_event. Amendment 3: both draft_order queries filter AND deleted_at IS NULL (collateral fix for latent on-clock ghost-row bug; mirror invariant with completion SUM). N-2 / KI-034 (2026-08-08, post-F27): completion UPDATE also clears leagues.draft_state to ''completed'' — Amendment 2''s "column deliberately untouched" ruling superseded by F27''s ignition path setting draft_state=''active''. Snake/linear only; auction completion is a separate future chunk (KI-029 named residual on auction ledger).';
