-- Fix xG calculation in populate_player_weekly_stats function
-- The current subquery joins raw_shots with player_game_stats again, which might cause issues
-- Simplify to directly filter raw_shots by joining with nhl_games for game_date
...
COMMENT ON FUNCTION public.populate_player_weekly_stats IS 'Populates weekly stats for a specific week from player_game_stats (fixed xG calculation to use nhl_games directly)';
