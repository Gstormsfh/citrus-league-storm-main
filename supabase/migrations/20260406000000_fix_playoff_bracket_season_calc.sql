-- Fix: generate_playoff_bracket used EXTRACT(YEAR FROM NOW()) which returns
-- the calendar year (2026 in April 2026), but CURRENT_SEASON in the app is 2025
-- (representing the 2025-26 NHL season). NHL seasons span two calendar years,
-- so we derive season as: current year if Oct-Dec, otherwise current year - 1.
...
DECLARE
  ...
  v_week_offset INT;
BEGIN
  -- Calculate NHL season: Oct-Dec = current year, Jan-Sep = previous year
  -- This matches CURRENT_SEASON in packages/shared/src/constants/season.ts
