-- Fix: dropped players never landed on waivers.
--
-- The existing trigger `trigger_track_dropped_players` fires on
-- `roster_transactions` (table doesn't exist / not written by the RPC),
-- but `process_roster_move` writes to `transaction_ledger`. Result: the
-- waiver trigger never fired, and `player_waiver_status` stayed empty —
-- any dropped player could be instantly re-added by anyone.
--
-- Fix: insert directly into `player_waiver_status` inside the RPC itself
-- on the drop leg. This is more reliable than a trigger (no dependency on
-- audit-log tables) and runs inside the same subtransaction as the drop,
-- so a rollback of the drop also rolls back the waiver entry.
...
BEGIN
  ...
  BEGIN
    IF p_drop_player_id IS NOT NULL THEN
      ...
      -- Put the dropped player on waivers (industry standard: 48h claim period
      -- unless overridden by leagues.waiver_period_hours). Idempotent via
    ...
    IF p_add_player_id IS NOT NULL THEN
      ...
      -- Revive soft-deleted draft_pick row for this player, or insert a new one.
...
COMMENT ON FUNCTION public.process_roster_move IS
  'Atomic add/drop. Drops now enter player_waiver_status (48h default). Goalie cap = GREATEST(starting_G + 2, 4).';
