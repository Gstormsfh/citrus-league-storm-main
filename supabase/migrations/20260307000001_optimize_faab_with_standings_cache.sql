-- ============================================================================
-- OPTIMIZE FAAB PROCESSING WITH STANDINGS CACHE
-- ============================================================================
-- Rewrites process_faab_waivers_for_league() to use the team_standings
-- materialized view instead of the LATERAL subquery that scanned the
-- entire matchups table per team per bid.
--
-- Before: O(pending_players × bids_per_player × completed_matchups)
-- After:  O(pending_players × bids_per_player) with O(1) standings lookup
--
-- Also refreshes standings at the start of each processing run to ensure
-- tiebreaker data is current.
-- ============================================================================
...
BEGIN
  ...
  -- CONCURRENTLY allows other reads to continue unblocked
  ...
  LOOP
    ...
    --
    -- OPTIMIZATION: Uses team_standings materialized view instead of
    -- LATERAL subquery scanning matchups table per team
    FOR v_bid IN
      SELECT
        wc.id AS claim_id,
        wc.team_id,
...
BEGIN
  RAISE NOTICE '';
  RAISE NOTICE '════════════════════════════════════════════════════════════════';
  RAISE NOTICE '  FAAB PROCESSING OPTIMIZED WITH STANDINGS CACHE';
  RAISE NOTICE '════════════════════════════════════════════════════════════════';
  RAISE NOTICE '';
  RAISE NOTICE '  Changes:';
  RAISE NOTICE '    - Replaced LATERAL matchups scan with team_standings lookup';
  RAISE NOTICE '    - Added REFRESH MATERIALIZED VIEW at start of processing';
  RAISE NOTICE '    - Added SET search_path = public for security';
  RAISE NOTICE '    - Preserved: conditional drop retry, $0 bids, advisory lock';
  RAISE NOTICE '';
  RAISE NOTICE '  Performance:';
  RAISE NOTICE '    Before: O(players × bids × matchups) per league';
  RAISE NOTICE '    After:  O(players × bids) per league — O(1) standings';
  RAISE NOTICE '════════════════════════════════════════════════════════════════';
END $$;
