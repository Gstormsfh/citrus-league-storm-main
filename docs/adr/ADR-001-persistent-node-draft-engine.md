...
1. **Server restart drops live WebSocket connections briefly.** On deploy or crash, every connected client reconnects. Mitigation: chunk 11g.5's `last_seen_seq` resume protocol replays missed events from `draft_events` via the in-memory ring buffer; chunk 11g.7's startup recovery rebuilds in-memory state for active drafts via snapshot + replay. Brief reconnect blip, no draft progress lost.
...
3. **uWebSockets.js dependency on existing server.** Adds `uWebSockets.js` to the existing Hono server (running on its own port; coexisting with the Hono HTTP API). **Chunk 11g.0 (dependency compatibility verification) is the explicit go/no-go gate** for this. If the existing server's middleware, async patterns, or build setup conflict with WebSocket upgrades in a way that can't be cleanly resolved, the alternative (a) — separate Cloud Run service — is the documented fall-back, not a rewrite.
...
- Phase 2 server-side pick path (`server/src/services/DraftServiceV2.ts`) — kept during cutover; once chunk 11g.4 ships and the WebSocket pick path is verified, the HTTP path becomes a fallback for clients that can't open WebSockets.
- The Phase 0–4 Known Issues registry (`docs/RUNBOOKS/draft-engine-v2-known-issues.md`) — KI-001..KI-005 still apply. **KI-006 RESOLVED** at chunk 11g.10 by the in-memory candidate cache (verified by the latency harness). **KI-007 RESOLVED** at chunk 11g.9 when vendored code is deleted alongside the Edge Function infrastructure.
...
End of chunk 11g.4. First end-to-end pick path: client opens WebSocket, submits a pick, server validates against in-memory state, calls `submit_pick_v2`, broadcasts to all clients in the room.
...
### Chunk 11g.7 — "Does crash recovery work?"

End of chunk 11g.7. Worker is killed and restarted; on boot it loads all active drafts from snapshots in `draft_state` + replay of `draft_events`, rebuilds in-memory `LobbyManager` state, resumes timers. Connected clients reconnect via the chunk 11g.5 `last_seen_seq` resume protocol; no picks lost; no duplicates. Per-draft reload p95 ≤ 1500ms (Mandate target).

### Chunk 11g.10 — "Does it meet every Mandate target end-to-end?"

End of chunk 11g.10. Performance instrumentation harness runs against the deployed server. **This is the Phase 5 entry gate.** Sign-off requires every Mandate target met: manual pick p95 ≤ 300ms / p99 ≤ 500ms; autopick p95 ≤ 1000ms / p99 ≤ 2000ms; broadcast fanout p95 ≤ 200ms; draft state load p95 ≤ 1500ms; reconnection recovery p95 ≤ 2000ms; timer drift < 100ms.
...
- `docs/PHASE_4_5_PLAN.md` — chunks 11g.0–11g.10 implementation plan.
...
- `docs/RUNBOOKS/draft-engine-v2-known-issues.md` — Phase 0–4 KI registry. KI-001..KI-005 still apply; KI-006 RESOLVED at chunk 11g.10 (verified by latency harness); **KI-007 RESOLVED at chunk 11g.9** when vendored shared code is deleted.
