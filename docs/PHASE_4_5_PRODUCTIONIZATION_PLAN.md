...
| 4 | **Postgres as durability and disaster recovery, not hot path.** | ✅ Satisfied (synced 2026-05-19 at chunk 11g.10 sub-step 10e). | The principle holds: Postgres = durability via `draft_events` event log + `draft_picks_v2` / `auction_nominations` / `auction_budgets` projection tables; the hot path is uWS broadcast, not Postgres. The role pgmq played (cross-process scheduling) is now filled by LISTEN/NOTIFY (chunk 11g.7-7e) — also Postgres-native, also off the hot path, also durability-only. **CLAUDE.md §1.3 pattern #4 wording synced 2026-05-19 at chunk 11g.10 sub-step 10e** — now reads "event log and projection tables … LISTEN/NOTIFY (chunk 11g.7-7e) provides cross-process signaling," replacing the stale "pgmq scheduler" reference. The verbatim quote in §1.3 above this section is preserved as historical record of CLAUDE.md's wording when 10a shipped (2026-05-12); the live CLAUDE.md is the canonical source going forward. |
...
**Open item from this audit (CLOSED 2026-05-19 at chunk 11g.10 sub-step 10e):** CLAUDE.md pattern #4 wording. Resolved as part of the 10e runbook ship; CLAUDE.md now reflects the LISTEN/NOTIFY substitution. The audit row 4 above carries the closure annotation. See `docs/PHASE_4_5_PROJECT_PLAN.md` Decision Log entry 2026-05-19 for the full sync rationale.
...
### R10F — CLAUDE.md staleness creep (low probability, low impact) — **CLOSED 2026-05-19 at chunk 11g.10 sub-step 10e**
CLAUDE.md §1.3 pattern #4 referenced pgmq — stale post-11g.9. The §4 audit caught this. **Resolution:** synced 2026-05-19 at chunk 11g.10 sub-step 10e as part of the runbook-rewrite ship. CLAUDE.md §1.3 pattern #4 now reads "event log and projection tables … LISTEN/NOTIFY (chunk 11g.7-7e) provides cross-process signaling," accurately reflecting the post-11g.9 architecture. The verbatim pre-edit quote in §1.3 of this plan doc is preserved as historical record of the wording when 10a shipped; §4 row 4 above carries the closure annotation. Original 10a positioning ("next CLAUDE.md touch") was conservative; the brief for 10e expanded scope to include this fix, on the rationale that runbook content references the architecture and stale CLAUDE.md wording would propagate confusion into operational docs.
