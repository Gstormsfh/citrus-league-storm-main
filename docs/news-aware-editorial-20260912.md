...
- `profile.ts` selects a lead from supplied evidence: combined assists/shooting, goal-led scoring, power-play exposure, assists, shots, physical categories, or goalie ratios/workload. Identity alone no longer changes prose. Supplied league categories and weight signs/magnitudes change the relevant advice. Historical component comparisons use only supplied stats, identify penalties, and preserve the hockey mechanism; they are not fantasy projections. Missing settings remain conditional. Actuals use their own source season, including historical and unknown-season cases. Totals do not establish line assignments, coach intent, job security, or inevitable finishing regression.
...
The modal's fallback receives attached raw news and configured scoring weights, and displays source links when available. Current assessments expose editorial version and source context for inspection. Server assessments are assembled on every request, outside the two-minute raw dashboard/history cache.
...
- Open player hooks refresh each minute and on changed news/scoring/source-season revision. Changed player or league identity immediately masks the previous response; magnitude-only scoring changes also clear prior copy; stale requests cannot win. Stable polling retains the existing server copy. Disabled/invalid players do not poll.
...
Run `npx tsx scripts/editorial/evaluate.ts` for [six before/after examples](editorial-evaluation/before-after.md), using public NHL 2025-26 actuals captured solely as frozen evaluation fixtures. Four contrasting forwards move from one identical analysis to four distinct analyses after numbers are removed. A separate clearly synthetic practice report demonstrates how news changes the conclusion. Interaction tests vary finishing at fixed shot volume, power-play share at fixed points, workload evidence, and the same report across different scoring profiles; unique strings alone are not the quality test. Fixtures are not runtime data sources.

Behavioral coverage includes source-season preservation, settings sensitivity, same-player news counterfactuals, wrong-player/roundup subjects, date cutoffs, supersession, injection/negation, no GP changes, independent news failure, server/bundle parity, and hook refresh races. The skill's bundled validator also passes. Final editorial branch checks: 429 shared tests, 165 targeted server tests, and 560 targeted web tests passed. Shared/server/web TypeScript checks, skill validation and diff whitespace checks passed. The projection reconciliation task owns final combined validation against its newer universal scoring work.
...
Canonical context now comes from the reconciliation task's active published-player view and travels through the same dashboard row. The shared adapter requires run/revision identity, preserves original source clocks and source links, rejects expired verification, and labels imported availability and role assumptions explicitly. A verified status can change the immediate decision; an active listing is not medical clearance. TEAMNOTES text remains untrusted and cannot create a second forecast or override store. Publication of an active source was not performed here.
