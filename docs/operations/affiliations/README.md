...
Finalsz has two standard IR slots. Current roster validation accepts the explicit eligibility flag or raw IR/LTIR designation; the player API uses the same rule. OUT/INJ/DTD/SUSP/unknown alone do not qualify. No IR+ eligibility policy was found or added. Existing IR occupants remain tolerated after activation, while failed eligibility/history reads cannot authorize a new placement. Tests verify IR/LTIR persistence, noneligible status rejection and configured capacity. No production roster moves were performed for QA.

Medical display evidence is separate: an owner-baseline LTIR badge alone does not establish fantasy IR eligibility. At this audit the raw talent feed contained no IR/LTIR records. Do not tell users that every medical badge is currently movable into IR.
