...
| **Machine type (chunk 11g.2 staging — production-target)** | **`e2-medium` or `e2-standard-2` minimum, NOT `e2-standard-4`.** | Spike measured 15.5 MiB container memory at idle on `e2-medium` (3.83 GiB available) — 0.40% utilization. Even projecting 100× scale-up for real `LobbyManager` instances, ~1,000 connections, and ~100 active drafts, ~1.5 GiB is the upper-bound estimate. `e2-medium`'s 4 GiB ceiling has comfortable headroom. `e2-standard-4` (16 GiB) was overprovisioned by an order of magnitude in the original spec. Cost implication: ~$50/mo staging+prod pair on `e2-medium` vs ~$200/mo on `e2-standard-4`. Real load testing 6 weeks pre-launch (per `PHASE_4_5_PROJECT_PLAN.md` Phase D) refines the final sizing. |
| **Region** | Provisional `northamerica-northeast1` (Montreal). **Decision deferred** — see §6. | Iowa-from-Edmonton beat Montreal-from-Edmonton on a single-user benchmark by ~5ms average, but one Canadian city's data is insufficient to flip the architectural default. Multi-city benchmark + Citrus's actual user geography needed before locking. Surface to Zach for ratification. |
...
**Decision: REGIONAL DECISION DEFERRED.** A single-user benchmark from one Canadian city is insufficient to flip the architectural default from Montreal. Chunk 11g.2 should:

1. Run a multi-city benchmark (Edmonton + Toronto + Vancouver + at least one US city) before locking the region.
2. Cross-reference with Citrus's actual user geography (where do beta users live? Where will marketing focus?).
3. Surface to Zach for ratification.

Until that broader benchmark, **Montreal remains the provisional default** — but chunk 11g.2 owners should not assume it. The Iowa-from-Edmonton finding is a real signal worth investigating further.
