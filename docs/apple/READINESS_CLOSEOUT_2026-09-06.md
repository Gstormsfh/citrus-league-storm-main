...
- The staging browser walkthrough exposed a sign-out/navigation race: the protected profile route redirected to login before the confirmation appeared. Sign-out now runs after the public confirmation route mounts, with pending and retry states so failed device cleanup never falsely appears complete.
...
- Full web suite after the deletion-route fix and final deletion wording correction: **4,465 passed across 306 files**. Web build, both TypeScript checks, server/shared suites, and source lint were rerun successfully. Deletion-page tests cover pending cleanup, failure/retry, direct visits, and StrictMode behavior.
...
1. Staging API and web deployment succeeded at `cdee0816`, followed by passing integrated checks below. The follow-up deletion-route fix at `60127a88` also deployed successfully and passed the fresh-bundle browser retest. Staging migrations `20260906164419`, `20260906164444`, `20260906165538`, and `20260906171108` are applied. Production deployment and migration activation remain held for the separately approved release.
...
4. Staging policies were byte-compared with the checked source, then both existing policy-version rows were activated as `2026-09-06`. API fixtures verified missing/outdated/current states; the browser displayed the new consent gate and the accepted versions as Active. Production policy publication/version activation remains held. Never manufacture real user acceptance.
...
The existing Apple key was found locally. A newly signed client secret and encryption key were prepared in an owner-only file outside the repository, then installed in staging Secret Manager during the authorized activation pass below. An intentional invalid-refresh-token probe returned Apple's `invalid_grant` response rather than `invalid_client`. This is a limited configuration probe, not proof of successful OAuth, token retention or revocation for a real account.
...
Google Cloud operator authentication was renewed. The first deployment run passed its test gate but failed while trying to inspect Secret Manager: the deployment account has no secret-administration permission. The authorized operator session successfully provisioned all three Apple secrets in `citrus-fantasy-staging`, with runtime access. Deployment was restarted using the existing service-account permissions. Provisioning remains a separate operator step; redundant temporary GitHub Apple inputs were removed. The script sends secret values through stdin and refuses an unplanned encryption-key replacement. Three operator-script tests passed.

Staging was missing the avatar bucket and its ownership policies. Migration `20260906165538` adds the missing policies without replacing existing named policies. The disposable integration script creates the bucket through the Storage API when needed. Run `python3 scripts/verification/run-apple-staging-e2e.py` with an authorized operator session; the wrapper reads only staging credentials into memory. The checks cover consent, reports/admin denial, block/unblock, suspension, avatar ownership and downloadable-byte removal, member deletion, commissioner succession, and orphaned-league deletion. The runner cleans up its fixtures on success or assertion failure.

Apple is now enabled in staging with client ID `com.citrussports.web`. Supabase's redirect allowlist includes the native sign-in and password-recovery URLs. The Apple Services ID now includes staging's Supabase domain and callback, preserving the existing production entries. The staged app reaches Apple's sign-in page without a callback error. Actual Apple authentication, retained-token verification, and revocation are paused until a suitable test identity is agreed. No second Apple account was created; setup used the existing Developer account. A regular production Citrus Apple identity must not be used for the destructive test without understanding its authorization scope.

Successful deployment: [run 34047274834](https://github.com/Gstormsfh/citrus-league-storm-main/actions/runs/34047274834), Cloud Run revision `citrus-api-00136-kt2`. All three Apple secret bindings are present. Both staged policy documents exactly match the repository source.

Live integration results: consent, membership denial, sender identity, reporting, admin denial, blocking/unblocking, suspension, avatar ownership and deletion, member deletion, commissioner succession, and orphan cleanup passed. The first pass found staging's missing `push_notifications` column; the parity migration fixed profile reads. The download assertion uses the actual public URL and waits for cache invalidation: the cached avatar stopped serving after 62 seconds. Storage-list removal alone was not treated as byte-removal proof. [Supabase documents eventual CDN invalidation and separate browser caches](https://supabase.com/docs/guides/storage/cdn/smart-cdn).

The browser pass verified email sign-in, the September 6 consent gate, persisted push opt-out after reload, and actual account deletion. It also reproduced the confirmation-route race described above; the deployed fix passed a fresh-bundle retest: deletion remained on `/account-deleted`, displayed the signed-out confirmation, and kept Apple follow-up instructions visible. All API/UI fixture accounts and leagues were confirmed absent afterward. No real account was deleted. Integration log: `/tmp/citrus-apple-staging-e2e.log`.
...
## Final candidate and environment boundary

Production API and the latest successful Production Deploy (run `34014460215`) both identify commit `0700055c296b44658b1d29b5d30911fb4b5215d3`, the base of this readiness branch. This establishes source ancestry, not environment equivalence. Staging data, provider configuration, migrations, and policy activation differ from production. Production remains unchanged.

The functional deletion fix deployed successfully in [run 34048670169](https://github.com/Gstormsfh/citrus-league-storm-main/actions/runs/34048670169) at `60127a88`. The browser retest used the refreshed hosting entry bundle, avoiding an older cached PWA bundle. Final copy-only corrections describe commissioner succession and orphan-league deletion accurately; these are verified locally and included in the rebuilt native archive, but are not yet deployed to staging.

Final local validation repeated the web build, web/server/shared suites, both TypeScript checks, and source lint successfully after those wording changes. Logs: `/tmp/citrus-apple-final-copy-build.log`, `/tmp/citrus-apple-final-copy-web.log`, `/tmp/citrus-apple-final-copy-server.log`, `/tmp/citrus-apple-final-copy-shared.log`, `/tmp/citrus-apple-final-copy-types.log`, `/tmp/citrus-apple-final-copy-server-types.log`, and `/tmp/citrus-apple-final-copy-lint.log`. The final archive succeeded and passed strict signature verification and byte comparison against synchronized native assets and the privacy manifest. All disposable API/UI accounts and test leagues were confirmed absent.
