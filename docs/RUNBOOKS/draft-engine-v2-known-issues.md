...
| **Target phase for resolution** | Operational. Resolution = (a) deploy-pipeline assertion that `SUPABASE_DB_URL` doesn't match pooled patterns — **shipped at chunk 11g.10 sub-step 10b** via `scripts/staging-deploy-checks.sh` + inline check in `infra/gce/draft-engine-startup.sh`; AND (b) on-call awareness via this KI + `draft-engine-v2-staging-preflight.md` §3.4. |
