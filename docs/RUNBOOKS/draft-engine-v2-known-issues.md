...
| **Target phase for resolution** | **Phase 8a** (prod cutover). The prod cutover migration replaces the literal URL with `'https://' \|\| (SELECT decrypted_secret FROM vault.decrypted_secrets WHERE name = 'draft_autopick_worker_url') \|\| '/functions/v1/draft-autopick'` (or equivalent). Same Vault provisioning recipe as `draft-autopick-token`; same rotation procedure. The runbook's "STAGING_PROJECT_REF" marker comment in the chunk 10d migration flags the line that must change. (Vault read API: Supabase Vault does NOT expose a `vault.read_secret(text)` function — the canonical name-based read is the `vault.decrypted_secrets` view. Earlier drafts of this doc and chunk 10d cited the non-existent function; corrected in the chunk 11e vault-fix follow-up.) |
| **Verification test** | After the Phase 8a cutover migration applies on prod: `SELECT count(*) FROM cron.job WHERE jobname = 'draft-autopick-keepalive' AND command LIKE '%jjgspcpvqaiitloglxbb%';` must return **0** (no staging ref leaked into prod). Plus the positive assertion `SELECT count(*) FROM cron.job WHERE jobname = 'draft-autopick-keepalive' AND command LIKE '%vault.decrypted_secrets%';` must return **1** (URL is Vault-resolved via the canonical view). Both assertions fail-loud during the cutover smoke test. |
...
### KI-007 — Phase 4 Edge Function uses vendored copies of shared code

| | |
|---|---|
| **Severity** | medium |
| **Function / file** | `supabase/functions/_shared/_vendored/{season,autopickConstants,draftHash,scoring}.ts` (vendored copies) and `supabase/functions/_shared/citrus_shared.ts` (re-export shim that imports from `./_vendored/` instead of `../../../packages/shared/`). Canonical sources: `packages/shared/src/constants/season.ts`, `packages/shared/src/constants/autopickConstants.ts`, `packages/shared/src/utils/draftHash.ts`, `packages/shared/src/utils/scoring.ts`. |
| **Description** | Phase 4's Edge Function (`supabase/functions/draft-autopick`) needs `ScoringCalculator`, `DEFAULT_SCORING`, `computePickPayloadHash`, `AUTOPICK_NAMESPACE_UUID`, `CURRENT_SEASON` from `@citrus/shared`. The chunk 11b smoke-deploy validated relative file imports out of `supabase/` (one constant, succeeded on the operator's box). The chunk 11d expanded imports failed at deploy time on Windows: the Supabase CLI bundler resolves relative paths Unix-style and errors when traversing out of `supabase/` on Windows hosts (`failed to read file: open C:\Users\...\packages\shared\...` with a `file:///Users/...` URL). To unblock deploys we vendored the four files into `supabase/functions/_shared/_vendored/` so every Edge Function import stays inside `supabase/`. **Risk:** silent drift between the canonical copy in `packages/shared/` (used by Node — server, web, tests) and the vendored copy (used by Deno — Edge Function). Either side could change without the other being updated, producing different FPTS values, different payload hashes, different idempotency keys, or stale season identifiers across runtimes. |
| **Why deferred** | Vendoring is the immediate unblock; a CI check that enforces parity is the durable fix but requires a separate design pass (where to run it — pre-commit hook, GitHub Actions, both? — and how to ignore the sync-on-change headers in the diff cleanly). Wedging that in Phase 4 risks scope creep into the worker chunks that need to ship now. |
| **Target phase for resolution** | **Phase 7** (load testing / hardening). The Phase 7 work adds a CI check (suggested: `.github/workflows/vendored-sync-check.yml`) that diffs each vendored file against its canonical source, ignoring the sync-on-change header block at the top of each vendored file. Workflow fails the PR if any pair drifts. Alternative: a pre-commit hook running `scripts/check-vendored-sync.sh` that does the same diff locally. Either suffices; CI is preferred for unmissable enforcement. |
| **Verification test** | Manual until Phase 7 lands: when modifying any of the four canonical files, immediately apply the same change to the corresponding `supabase/functions/_shared/_vendored/` file. After the Phase 7 CI check ships: deliberately introduce a drift (e.g. change `DEFAULT_SCORING.skater.goals` from 3 to 4 in `packages/shared/src/utils/scoring.ts` only), open a PR, and verify the CI check fails with a diff output pointing at the offending file pair. Then revert and verify the PR goes green. |
