...
def match_shots(our_shots, moneypuck_shots, coord_tolerance=3.0):
    """
    Match shots between our data and MoneyPuck data.
    ...
    # Convert MoneyPuck coordinates to our coordinate system if needed
    # MoneyPuck uses: xCord, yCord (may need adjustment)
    # Our system uses: shot_x, shot_y (NHL coordinates, net at x=89, y=0)
    
    # Match by coordinates (game IDs don't match between datasets)
    # We'll match all shots by coordinates within tolerance
    print("Step 1: Matching by coordinates (game IDs use different formats)...")
    matched = []
    unmatched_ours = []
    
    # Try to match by date if available, otherwise match all shots
    ...
    if our_has_date and mp_has_date:
        # Extract date from our data
        our_shots['match_date'] = pd.to_datetime(our_shots['created_at']).dt.date
        # Extract date from MoneyPuck (try different column names)
    ...
    # Group by date if available, otherwise process all at once
    if group_by_date:
        ...
        for match_date in dates_in_both:
            ...
            # For each of our shots, find closest MoneyPuck shot
            ...
            best_distance = float('inf')
                ...
                # Get MoneyPuck coordinates
                # Note: May need to adjust based on their coordinate system
                ...
                match_record = {
                    ...
                    total_matched += 1
                    # Remove matched MoneyPuck shot to avoid duplicate matches
                    mp_date_shots = mp_date_shots.drop(idx_mp)
                else:
        ...
        # No date grouping - match all shots (slower but works)
        ...
        for idx_our, row_our in our_shots.iterrows():
            ...
            if best_match:
                idx_mp, row_mp = best_match
                mp_used.add(idx_mp)
                # Create matched record (same as above)
                match_record = {
                    'our_game_id': row_our['game_id'],
                    'our_player_id': row_our['player_id'],
                    'our_shot_x': our_x,
