...
import numpy as np
import pytest
import run_timing_ridge10 as runner


def test_primary_control_is_ridge100_not_older_expanding():
    losses={n:{m:.1 for m in runner.METRICS} for n in runner.OUTPUTS}
    losses['expanding']={m:.2 for m in runner.METRICS}
    losses['timing']={m:.11 for m in runner.METRICS}
...
def test_scalar_independent_nonzero_offsets_clips_and_unseen_categories():
    ...
    model={'contract':runner.engine.VERSION,'settings':deepcopy(runner.engine.SETTINGS),'publishable':False,
        ...
        'timing_offsets':[.7,-.6],'timing_ridge':10.,
