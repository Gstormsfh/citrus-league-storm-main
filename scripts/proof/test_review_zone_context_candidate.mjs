...
import {MODELS,parseArgs,verifyBaseline,reviewFold,primaryGuard} from './review_zone_context_candidate.mjs';
...
test('primary guard independently requires both metrics against both controls',()=>{
  const overall=Object.fromEntries(['zone_monotone_group','frozen_monotone_group','frozen_movement_monotone_group'].map(n=>[n,{brier:.06,log_loss_clipped:.22}]));
...
function fixture(){
  const rows=[0,1,0].map((target,i)=>({game_id:i===2?2:1,event_id:i+1,target,
    groups:{strength:i===2?null:'5v5'},predictions:{frozen_monotone_group:.5,frozen_movement_monotone_group:.4,
      zone_raw:target?.8:.2,zone_sigmoid:target?.9:.1,zone_monotone_group:target?.75:.25}}));
  const prior=rows.map(r=>({...r,predictions:{monotone_logit_group:r.predictions.frozen_monotone_group}}));
  const movementPrior=rows.map(r=>({...r,predictions:{movement_monotone_group:r.predictions.frozen_movement_monotone_group}}));
  function block(rr){
...
test('review independently reconciles points, bins, subgroups and frozen identity',()=>{
  ...
  assert.equal(r.overall.zone_monotone_group.brier,.0625);
...
for(const [name,mutate] of [
  ...
  ['missing candidate',x=>{delete x.rows[0].predictions.zone_raw;}],
...
for(const [name,mutate] of [
  ['overall loss',x=>{x.card.overall.models.zone_raw.metrics.brier.value+=.01;}],
  ['bin population',x=>{x.card.overall.models.zone_raw.reliability[0].events+=1;}],
  ['subgroup loss',x=>{x.card.subgroups[0].statistics.models.zone_raw.metrics.brier.value+=.01;}],
...
test('argument contract requires separate scoped run and create-only output locations',()=>{
  const results=path.join(ROOT,'scripts/proof/results'),run=path.join(results,'official-zone-context-test'),
    output=path.join(results,'zone-context-candidate-review-test');
  ...
  assert.throws(()=>parseArgs(['--run','/tmp/official-zone-context-test','--output',output]));
  assert.throws(()=>parseArgs(['--run',run,'--output',path.join(run,'zone-context-candidate-review-test')]));
});
