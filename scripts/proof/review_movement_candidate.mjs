...
export const MODELS=['frozen_monotone_group','movement_raw','movement_sigmoid','movement_monotone_group'].sort();
const FOLDS=['fold1','fold2'],PRIMARY='movement_monotone_group',FROZEN='frozen_monotone_group';
...
export function parseArgs(args){
  ...
  assert(path.basename(run).startsWith('official-movement-'));
  assert(path.basename(output).startsWith('movement-candidate-review-'));
  assert.notEqual(run,output);return {run,output};
}

export function verifyBaseline(rows,prior){
...
export function reviewFold(rows,card,prior){
  verifyBaseline(rows,prior);
  const report=auditCard(rows,card);
  report.baseline_parity='exact_event_label_groups_probability';
  report.primary=PRIMARY;
  report.primary_guard_passed=['brier','log_loss_clipped'].every(m=>report.overall[PRIMARY][m]<=report.overall[FROZEN][m]);
...
export function review({run,output}){
  ...
  try{
    ...
    for(const name of ['review_movement_candidate.mjs','test_review_movement_candidate.mjs','review_method_challengers.mjs']){
    ...
    assert.equal(health.status,'complete-movement-development-not-accepted');assert.equal(health.publishable,false);
    ...
    for(const fold of FOLDS){
      ...
      const report=reviewFold(read(path.join(run,fold,'predictions.json')),
        read(path.join(run,fold,'scorecard.json')),read(priorFile));
    ...
    const report={contract:'citrus-independent-js-movement-review-v1',status:'independent-point-review-complete',
