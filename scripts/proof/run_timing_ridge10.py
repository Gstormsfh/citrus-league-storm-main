...
import timing_conditional_calibration_ridge10 as engine
...
PLAN = 'docs/analytics-timing-ridge10-plan-20260906.md'
PLAN_SHA = '9153733a6251fb85726ae577d5a150f7e7bc713a000d82df5a89edff29c8a635'
...
CODE = ('scripts/proof/run_timing_ridge10.py', 'scripts/proof/timing_conditional_calibration_ridge10.py',
    'scripts/proof/test_timing_ridge10.py', 'scripts/proof/test_timing_ridge10_runner.py')
CONTROL = 'scripts/proof/results/official-timing-candidate-20260906-full'
CONTROL_SHA = 'f035ec542fc02d7b91f9ec3f66cd8218c3746bb095f47dc31ace7ebc89e49ed9'
METRICS = ('brier', 'log_loss_clipped')
OUTPUTS = ('fixed', 'expanding', 'timing100', 'timing')
...
def run(output):
    ...
    if (output.parent != ROOT/'scripts/proof/results' or not output.name.startswith('official-timing-ridge10-')
    ...
    try:
        ...
        pin_run(closure, CONTROL, CONTROL_SHA, 'complete-timing-candidate-development-not-accepted')
        closure.mapping(closure.read(CONTROL+'/consumed-file-sha256.json'))
        ...
        for fold in ('fold1', 'fold2'):
            ...
            controls = closure.read(f'{CONTROL}/{fold}/predictions.json')
            if set(strict_keys(controls)) != set(baseline): raise ValueError('Exact ridge100 control membership')
            for c in controls:
                prior = baseline[c['game_id'], c['event_id']]
                if c['target'] != prior['target'] or c['groups'] != prior['groups'] or any(c['predictions'][n] != prior['predictions'][n] for n in ('fixed','expanding')):
                    raise ValueError('Ridge100 baseline source drift')
                prior['predictions']['timing100'] = c['predictions']['timing']
        ...
        save('declaration.json', {'plan_sha256': PLAN_SHA, 'source_health_sha256': SOURCE_SHA,
            'timing_health_sha256': TIMING_SHA, 'control_health_sha256': CONTROL_SHA, 'checked_sha256': dict(closure.checked), 'preflight': bounds,
        ...
        for fold, item in prepared.items():
                ...
                def score(rows, split):
                    lineage = {'prediction_rows_sha256': fingerprint(rows), 'source_manifest_sha256': source_manifest_sha,
                        'split_sha256': fingerprint(split), 'pipelines': {n: fingerprint({'source': SOURCE_SHA,
                            ...
                            'control': CONTROL_SHA, 'name': n}) for n in OUTPUTS}}
            ...
            control_losses = closure.read(f'{CONTROL}/{fold}/summary.json')['losses']['timing']
            if any(abs(losses['timing100'][m]-control_losses[m]) > 1e-12 for m in METRICS): raise ValueError('Ridge100 point loss drift')
        ...
        save('health.json', {'status': 'complete-timing-ridge10-development-not-accepted', 'publishable': False, 'files': dict(files)})
