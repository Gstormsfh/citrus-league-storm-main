...
const PLAN='docs/analytics-timing-candidate-plan-20260906.md',PLAN_SHA='406084699d40ddfc4b6c735ea6f0cdfd2d5188f131734fde0b85098d8595cb8c';
...
const NAMES=['fixed','expanding','timing'],METRICS=['brier','log_loss_clipped'];
...
export function scalarPrediction(model,p,c,gap){
 assert.equal(model.contract,'citrus-timing-conditional-calibration-v1');assert.equal(model.timing_ridge,100);assert.equal(model.publishable,false);
...
export function verifyPredictions(rows,test,model,frozen){
 ...
 for(const r of rows){const k=key(r);assert(!seen.has(k)&&inputs.has(k),'Exact unique predictions');seen.add(k);const x=inputs.get(k),prior=frozen.get(k);assert(prior);assert.equal(r.target,x.target);assert.equal(r.target,prior.target);assert.deepEqual(r.groups,prior.groups);assert.equal(Object.keys(r.groups).length,8,'Preserve original eight groups');assert.deepEqual(Object.keys(r.predictions).sort(),[...NAMES].sort());for(const n of NAMES)probability(r.predictions[n]);for(const n of ['fixed','expanding'])assert.equal(r.predictions[n],prior.predictions[n],'Frozen control drift');const error=Math.abs(r.predictions.timing-scalarPrediction(model,x.raw_probability,x.context,x.gap_seconds));assert(error<=1e-12,'Scalar prediction drift');maxError=Math.max(maxError,error);}
 return maxError;
}
export function guard(losses){for(const n of ['timing','expanding'])for(const m of METRICS)assert(finite(losses[n][m])&&losses[n][m]>=0);return METRICS.every(m=>losses.timing[m]<=losses.expanding[m]);}
...
export async function review(run,output){
 run=path.resolve(run);output=path.resolve(output);const parent=path.join(ROOT,'scripts/proof/results');assert.equal(path.dirname(run),parent);assert(path.basename(run).startsWith('official-timing-candidate-'));assert.equal(path.dirname(output),parent);assert(path.basename(output).startsWith('timing-candidate-review-'));
 ...
 try{
 const checker={};for(const n of ['review_timing_candidate.mjs','test_review_timing_candidate.mjs','review_calibration_transfer.mjs','review_calibration_stability.mjs','review_forward_shooter_movement.mjs','review_method_challengers.mjs','review_timing_eras.mjs']){const p=path.join(ROOT,'scripts/proof',n);checker[n]=hash(bytes(p));}
 const health=read(path.join(run,'health.json'));assert.equal(health.status,'complete-timing-candidate-development-not-accepted');bindInventory(reader,run,health);const sourceHealthSHA=hash(bytes(path.join(run,'health.json')));
