...
from projections import identity_probability_v2 as candidate
from projections import identity_probability as original_candidate
...
PLAN = 'docs/analytics-identity-probability-v2-plan-20260906.json'
FAILED = 'scripts/proof/results/official-identity-probability-20260906-1731'
...
CODE = ['scripts/proof/run_identity_probability_v2.py', 'scripts/proof/test_run_identity_probability_v2.py',
        'data-pipeline/projections/identity_probability_v2.py', 'data-pipeline/tests/test_identity_probability_v2.py',
        'docs/analytics-identity-probability-plan-20260906.json', 'scripts/proof/run_identity_probability.py', 'scripts/proof/test_run_identity_probability.py',
...
def validate_plan(plan, now):
    if (plan.get('contract') != candidate.VERSION + ':plan' or plan.get('baseline') != NAMES[0]
            ...
            or fingerprint(plan.get('numerics')) != fingerprint(candidate.NUMERICS)
...
def run(output):
    ...
    if (output.parent != ROOT/'scripts/proof/results' or not output.name.startswith('official-identity-probability-v2-')
    ...
    try:
        ...
        failed = read(FAILED+'/failure.json', plan['v1_failure_sha256'])
        if failed['status'] != 'failed-identity-experiment': raise ValueError('Retained failed v1 evidence required')
        preserved = {p.relative_to(ROOT/FAILED).as_posix() for p in (ROOT/FAILED).rglob('*') if p.is_file()}
        if preserved != set(failed['files']) | {'failure.json'}: raise ValueError('Failed v1 membership changed')
        for name, digest in failed['files'].items(): read(FAILED+'/'+name, digest, parse=False)
        read(plan['numerical_diagnosis_path'], plan['numerical_diagnosis_sha256'], parse=False)
        for name, digest in plan['preserved_v1_code_sha256'].items(): read(name, digest, parse=False)
...
def diagnose(output):
    """Calibration-only numerical diagnosis; never opens validation inputs."""
    output = Path(output).absolute()
    if (output.parent != ROOT/'scripts/proof/results' or not output.name.startswith('official-identity-probability-v2-')
            or any(p.is_symlink() for p in (output, *output.parents))):
        raise ValueError('New scoped nonsymlink diagnosis directory required')
    output.mkdir(exist_ok=False)
    name = FAILED+'/fold1/calibration-identity-inputs.json'
    raw = safe(ROOT, name).read_bytes(); records = strict_json(raw)
    rows = prediction_inputs(records); yy = [r['target'] for r in records]
    original_candidate.validate_rows(rows)
    _, ix, ridge = original_candidate._design(rows, 'joint')
    pp = np.asarray([r['probability'] for r in rows])
    args = (np.log(pp)-np.log1p(-pp), np.asarray(yy), ix, ridge)
    with threadpool_limits(limits=1):
        initial = original_candidate.minimize(original_candidate.objective_gradient, np.zeros(len(ridge)), args=args,
            jac=True, method='L-BFGS-B', bounds=[(-1.5,1.5)]*len(ridge),
            options={'maxiter':1000,'ftol':1e-15,'gtol':1e-8,'maxls':40})
        value, gradient = original_candidate.objective_gradient(initial.x,*args)
        improved = candidate.fit(rows,yy,mode='joint')
    result = {'contract':candidate.VERSION+':calibration-only-numerical-diagnosis', 'publishable':False,
        'validation_opened':False, 'source_path':name, 'source_sha256':hashlib.sha256(raw).hexdigest(),
        'events':len(rows), 'parameters':len(ridge), 'v1':{'iterations':int(initial.nit),
        'success':bool(initial.success),'message':str(initial.message),'objective':value,
        'raw_projected_gradient':float(np.max(np.abs(original_candidate._projected_gradient(initial.x,gradient))))},
        'v2':improved['numerical_diagnosis'], 'code_sha256':{n:file_sha(safe(ROOT,n)) for n in CODE},
        'objective_bounds_penalties_and_raw_gate_unchanged':True}
    if file_sha(safe(ROOT,name)) != result['source_sha256']: raise ValueError('Diagnosis input drift')
    persist(output,'diagnosis.json',result)
    return {'status':'calibration-only-diagnosed-not-evaluated','diagnosis_sha256':file_sha(output/'diagnosis.json')}


def main():
    parser = argparse.ArgumentParser(description=__doc__); parser.add_argument('--output',required=True)
    parser.add_argument('--diagnose-calibration-only',action='store_true')
    ...
    print(json.dumps(diagnose(args.output) if args.diagnose_calibration_only else run(args.output)),flush=True)
