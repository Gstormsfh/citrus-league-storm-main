...
PLAN = 'docs/analytics-movement-candidate-plan-20260906.json'
CODE = ['data-pipeline/projections/pre_shot_history.py', 'data-pipeline/projections/pre_shot_movement.py',
...
VERSION = 'citrus-matched-pre-shot-movement-v1'
...
def primary_passed(losses):
    return all(losses['movement_monotone_group'][m] <= losses['frozen_monotone_group'][m]
               for m in ('brier','log_loss_clipped'))


def run(output):
    output=Path(output).absolute()
    if (output.parent!=ROOT/'scripts/proof/results' or not output.name.startswith('official-movement-')
    ...
    try:
        ...
        plan=read(PLAN)
        if (plan['contract']!=VERSION+':plan' or plan['primary']!='movement_monotone_group'
        ...
        save('feature-audit.json',{'names':NAMES,'availability':availability,'sampled_games_prefix_and_outcome_invariant':samples,
             ...
             'publishable':False})
        schema=deepcopy(replay.plan['schema']);schema['version']=VERSION;schema['names']+=list(NAMES)
        ...
        for fold,parts in enriched.items():
            print(json.dumps({'event':'movement.fit','fold':fold}),flush=True)
            ...
            predictions={'frozen_monotone_group':base,'movement_raw':pp['validation'],
                'movement_sigmoid':apply_map(pp['validation'],sigmoid),
                'movement_monotone_group':calibration_shape.predict(shape,pp['validation'],contexts['validation'])}
            evaluated=[]
            ...
            lineage={'prediction_rows_sha256':fingerprint(evaluated),'source_manifest_sha256':REVIEW_SHA,
                ...
                'pipelines':{n:fingerprint({'model_sha256':fingerprint(baseline_model if n=='frozen_monotone_group' else portable),
                    'map_sha256':fingerprint({'frozen_monotone_group':baseline_map,'movement_raw':None,
                        'movement_sigmoid':sigmoid,'movement_monotone_group':shape}[n]),
            ...
            print(json.dumps({'event':'movement.complete','fold':fold,**summaries[fold]}),flush=True)
        replay.verify()
        ...
        persist(output,'health.json',{'status':'complete-movement-development-not-accepted','files':files,'publishable':False})
