...
\echo '     step 4.5.'
\echo ''
\echo '  4.5. Synthetic repair exercise (architect addendum, 2026-08-06):'
\echo '         psql "$env:SUPABASE_DB_URL_PROD" -v ON_ERROR_STOP=1 \'
\echo '           -f scripts/proof/sl1b-synthetic-repair-exercise.local.sql'
...
\echo ''
\echo '  6. Post-heal verify (same file as v1 — shape-agnostic):'
\echo '       psql "$env:SUPABASE_DB_URL_PROD" -v ON_ERROR_STOP=1 \'
\echo '         -f scripts/proof/sl1-post-heal-verify.local.sql'
\echo '     Pre-registered acceptance:'
\echo '       missing_players_check              MUST be 0'
\echo '       team_lineups_vs_draft_picks_count  forks:'
\echo '         (A) 0 → check sums array lengths; SL-1b closes'
\echo '         (B) ≠0 → check is ROWS vs PICKS; unit-broken since Jan;'
\echo '                  new KI + check fix (NOT a v2 failure)'
\echo '       fantasy_daily_rosters_sync_today   unchanged 12 (KI-040)'
\echo ''
\echo '  7. Re-enable cron job 4 (auto-fix-integrity nightly 04:00 UTC):'
\echo '       ONLY after step 6 pre-registered acceptance holds.'
\echo '       Also pending KI-041 answer on who disabled it originally.'
\echo ''
