...
from projections import verified_movement_reuse as reuse
...
def run(mode,output,reference=None):
    ...
    names=['data-pipeline/projections/verified_movement_reuse.py','data-pipeline/tests/test_verified_movement_reuse.py',
           'scripts/proof/prove_verified_movement_reuse.py']
    ...
    try:
        ...
        if reference:
            ...
            if not reference.is_relative_to(ROOT/'scripts/proof/results') or reference.is_symlink():raise ValueError('Safe proof reference required')
            prior=strict_json(reference.read_bytes())
            if prior['comparison']!=proof or prior['mode']==mode:raise ValueError('Fast/slow full-row equivalence failed')
            equivalent=True
            closure[str(reference)]=hashlib.sha256(reference.read_bytes()).hexdigest()
