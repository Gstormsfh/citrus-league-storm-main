// Owns ONLY new, labeled, tmpfs Docker fixtures. No hosted credentials or volumes.
...
try {
  ...
  const pg = await docker('run', '--detach', '--label', `${label}=${owner}`, '--network', networkName,
    ...
    '--tmpfs', '/var/lib/postgresql/data:rw,size=1073741824', '--env', 'POSTGRES_HOST_AUTH_METHOD=trust', pgImage);
  ...
  assert(pgInfo.Mounts.some(m => m.Type === 'tmpfs' && m.Destination === '/var/lib/postgresql/data'));
  ...
  result = { status: health.status, owner, publishable: false, production_changed: false };
} catch (error) {
