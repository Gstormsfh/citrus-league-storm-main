...
export const MODELS=['frozen_monotone_group','frozen_movement_monotone_group','zone_raw','zone_sigmoid','zone_monotone_group'].sort();
const FOLDS=['fold1','fold2'],PRIMARY='zone_monotone_group',FROZEN='frozen_monotone_group';
...
export function parseArgs(args){
  ...
  assert(path.basename(run).startsWith('official-zone-context-'));
  assert(path.basename(output).startsWith('zone-context-candidate-review-'));
...
export function review({run,output}){
  ...
  try{
    ...
    for(const name of ['review_zone_context_candidate.mjs','test_review_zone_context_candidate.mjs','review_method_challengers.mjs']){
    ...
    assert.equal(health.status,'complete-zone-context-development-not-accepted');assert.equal(health.publishable,false);
    ...
    const report={contract:'citrus-independent-js-zone-context-review-v1',status:'independent-point-review-complete',
