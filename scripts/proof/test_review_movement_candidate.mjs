...
import {MODELS,parseArgs,verifyBaseline,reviewFold} from './review_movement_candidate.mjs';
...
function fixture(){
  const rows=[0,1,0].map((target,i)=>({game_id:i===2?2:1,event_id:i+1,target,
    groups:{strength:i===2?null:'5v5'},predictions:{frozen_monotone_group:.5,
      movement_raw:target?.8:.2,movement_sigmoid:target?.9:.1,movement_monotone_group:target?.75:.25}}));
...
])test('baseline guard rejects '+name,()=>{const x=fixture();mutate(x);assert.throws(()=>verifyBaseline(x.rows,x.prior));});
...
])test('arithmetic guard rejects changed '+name,()=>{const x=fixture();mutate(x);assert.throws(()=>reviewFold(x.rows,x.card,x.prior));});

test('argument contract requires separate scoped run and create-only output locations',()=>{
  const results=path.join(ROOT,'scripts/proof/results'),run=path.join(results,'official-movement-test'),
    output=path.join(results,'movement-candidate-review-test');
  ...
  assert.throws(()=>parseArgs(['--run','/tmp/official-movement-test','--output',output]));
  assert.throws(()=>parseArgs(['--run',run,'--output',path.join(run,'movement-candidate-review-test')]));
});
