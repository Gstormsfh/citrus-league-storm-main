// Infrastructure v2: explicit Mounts API plus independent live-kernel tmpfs proof.
...
const sourcePins = {};
for (const name of ['scripts/proof/run_local_model_publication_v2.mjs', 'scripts/proof/local_tmpfs_contract.mjs', 'scripts/proof/run_local_model_publication.mjs'])
...
try {
  ...
  const pgPort = Number(pgInfo.NetworkSettings.Ports['5432/tcp'][0].HostPort);
  let ready = false;
  for (let i = 0; i < 60; i++) {
    alive();
    try { await docker('exec', pg, 'pg_isready', '-U', 'postgres'); ready = true; break; } catch { /* bounded startup only */ }
  ...
  const restPort = Number(restInfo.NetworkSettings.Ports['3000/tcp'][0].HostPort);
  ready = false;
  for (let i = 0; i < 60; i++) {
    alive();
  ...
  result = { status: health.status, owner, infrastructure_version: 2, source_sha256: sourcePins, publishable: false, production_changed: false };
} catch (error) {
