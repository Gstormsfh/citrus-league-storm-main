...
def test_only_timing_penalty_changes_against_frozen_ridge100():
    import timing_conditional_calibration as old
    p,y,c,g,plan,theta=setup()
    theta[-4:]=[.2,-.4,.1,.3]
    args=(p,y,c,g,plan['vocabulary'],plan['states'],plan['timing_categories'])
    loss,grad=timing.objective(theta,*args)
    old_loss,old_grad=old.objective(theta,*args)
    base=plan['allocation']['base_design_width']
    assert old_loss-loss==pytest.approx(45*np.dot(theta[base:],theta[base:])/len(p),abs=1e-12)
    np.testing.assert_allclose(old_grad[:base],grad[:base],atol=1e-12,rtol=0)
    np.testing.assert_allclose(old_grad[base:]-grad[base:],90*theta[base:]/len(p),atol=1e-12,rtol=0)
    assert timing.SETTINGS==old.SETTINGS and timing.SETTINGS['group_ridge']==100
...
def model_fixture():
    ...
    return {'contract': timing.VERSION, 'settings': copy.deepcopy(timing.SETTINGS), 'publishable': False,
        ...
        'timing_categories': plan['timing_categories'], 'timing_offsets': [0.]*4, 'timing_ridge': 10.,
...
def test_dense_reference_and_penalty_once_total_row_normalized():
    ...
    expected_loss=(np.logaddexp(0,z).sum()-y@z+penalty+5*np.dot(theta[base:],theta[base:]))/len(p)
    expected_grad=(design.T@(expit(z)-y)+np.r_[grad,10*theta[base:]])/len(p)
