...
mkdir -p "$OUT_DIR"
tmp_schema=$(mktemp) ; tmp_cron=$(mktemp)
trap 'rm -f "$tmp_schema" "$tmp_cron"' EXIT
...
echo "prod_schema.sql: server pg$server_major, schemas=$SCHEMAS, $tables tables, $functions functions, $policies policies; prod_cron.sql: $jobs jobs"
