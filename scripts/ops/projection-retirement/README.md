...
Start with the [current source → screen operating map](CUTOVER.md). This is its detailed retirement evidence and guarded-rehearsal companion, not a current deployment report or blanket database cleanup.

The inventory below records the **pre-publication September12 read-only inspection**, including its then-current sizes, row counts and rollout prerequisites. Those observations are historical: canonical publication and code reader/helper retirement subsequently occurred. `projection_cache` remains intentionally retained and no production quarantine/drop occurred. Current serving identity and qualified acceptance live only in the map's linked final release receipt.
...
| canonical_projection_runs, canonical_projection_active, canonical_published_runs | Immutable reviewed source, derived runtime lineage, active pointer and published view. Original source_payload/source_revision remain separate from nightly runtime counts/revision. Absent at the original inspection; subsequently published (see current map). | KEEP; installation/publication subsequently completed. These implement one logical authority, not unnecessary copies. | Prior immutable source/run plus expected-active CAS; reconciliation owns materialization/activation rollback. |
...
| load_physical_projection, save_physical_projection, recalculate_fantasy_points_for_league | Two uncalled legacy cache helpers and a return0placeholder in calculate_daily_projections.py. No checked-in calls in either compared revision. | Removed in the subsequent reviewed code release, preserving all other module statements. Unknown external imports remain a compatibility limitation, not evidence of active use. | Git revert restores exact functions. No live writer/job changed. |
...
## Historical proposed quarantine sequence — not executed in production
...
Eight PGlite tests pass for rename/inverse and blocking unapproved gates, changed rows, incoming FKs, views, scheduled jobs, routine references and target collision. Run with `CITRUS_TEST_RUNTIME=/path/to/citrus node --test scripts/ops/projection-retirement/retirement.test.mjs` where the supplied runtime has @electric-sql/pglite. Full-schema PostgreSQL17.6 rehearsal with the actual backup passed in the reconciliation owner's isolated network-disabled container:1461rows, identical content MD5, same table/policy/index/constraint OIDs, preserved fixture ACLs/RLS, and valid real NHL parent rows. The inverse restored the original table name. Evidence: `/Users/gstorms/.codex/worktrees/4304/citrus/docs/verification/cache-retirement-full-schema-20260912.json`; reproducible script: `data-pipeline/tests/rehearsal/rehearse_cache_retirement.py` in that checkout. Evidence hashes match these exact SQL files and the retained backup. This proves local restore, not production release or external-client approval.

Integrated validation: reconciliation cherry-picked93e382fe as46004a7b; all66 focused Python projection/import tests and8 retirement SQL guards passed there. This closes the initial local pytest availability limitation. Full-schema actual-backup restore evidence is now recorded above; The code release subsequently completed; external-client gates for cache quarantine remain.
