...
## Secondary eligibility store — 2026-09-14

Owner decision 2026-09-14: build the protected manual secondary store now, then return Zuccarello to official C with a recorded RW grant.

Migration `20260914180000_governed_secondary_eligibility.sql` adds `player_eligibility_events` (append-only; `grant` / `revoke`, a revoke must supersede the grant it revokes; reason, sources and recorded_by required; SELECT for authenticated, INSERT for service_role only, no UPDATE/DELETE for anyone). `player_current_directory.eligible_positions` now resolves to the current primary plus every active grant, and exposes the active grants as `eligibility_events`. The raw `player_directory.eligible_positions` cell is ingestion evidence only: `sync_rosters.py` may still rewrite it, and that rewrite can no longer add or remove a fantasy position. At activation, season 2026 carried 11 non-NULL raw cells, all equal to the primary, so resolution changed no player's eligibility.

`restore-zuccarello-nhl-baseline-with-rw-grant.sql` performs the two-event owner decision in one transaction (restore_feed superseding `76fa0e35-…`, then an RW grant), with pre-image and post-image checks and an active-lineup conflict check; the one saved RW slot stays valid throughout. Test: `node --test scripts/ops/position-eligibility/secondary-eligibility.test.mjs`.

Candidates the seven-player audit above already flagged for an owner grant decision (each needs its own recorded event with sources): Duhaime LW, Olofsson LW, Marchessault RW, Laine LW, Schwindt RW. Draft guide exports must capture eligibility from `player_current_directory`, not the raw table, to include grants.
