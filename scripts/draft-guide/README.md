...
The effective-current edition verifies the exact PostgreSQL revision preimage with `--revision-preimage`, checks that its parsed object equals the runtime payload without `revision`, and accepts `--runtime-run-id` from the matching export receipt. It scores `remaining.used` and retains original full-season `exposure` separately. Both parent source and runtime identities, refresh timestamp and remaining-season as-of are shown. The source editor remains an editor of reviewed source assumptions; nightly runtime rates are derived data, not newly approved edits.

The 2026-09-12 runtime `858d8a9c` is the current export snapshot; activated10:55:47UTC, with inherited model refresh10:02:13UTC recorded separately. The earliera2b edition remains preserved; remaining and full schedule both contain 84 team games because this is preseason. Its refreshed rates differ from the preserved `1fb82cff` source edition. Later runtime refreshes require another explicit snapshot import. Nonzero configured categories without forecasts produce unavailable scores/ranks. Optional signed plus/minus is retained when supplied; loading a settings file that selects it exposes that weight in the preview.

## Current position eligibility

`build.py --eligibility /absolute/path/to/eligibility.json` accepts a separately fingerprinted directory snapshot with `schema_version: "citrus.position-eligibility.v1"`, integer `season`, ISO `as_of`, and `players` keyed by player ID. Each row carries `position` and `eligible_positions` (CSV or array). Export the governed `player_current_directory` values; do not manufacture secondaries from projection roles or old seasons. The adapter executes the same TypeScript reader as the app/server.

When supplied, POS prints current eligibility such as C/LW; the overlay fingerprint and date appear in the source appendix and manifest. Unmatched players retain only their projection primary. Position boards still group by projection primary and show overall ranks; canonical ranks, rates, scoring and team scenarios remain unchanged. `verify_canonical_pdf.py --eligibility` verifies the same binding. Without an overlay, existing labels retain their original primary rank suffix.
