...
The 2026-09-12 runtime `a2b5d057` is the current export snapshot; remaining and full schedule both contain 84 team games because this is preseason. Its refreshed rates differ from the preserved `1fb82cff` source edition. Later runtime refreshes require another explicit snapshot import. Nonzero configured categories without forecasts produce unavailable scores/ranks. Optional signed plus/minus is retained when supplied; loading a settings file that selects it exposes that weight in the preview.
