...
for (const group of ['skater', 'goalie']) {
  ...
  for (const [key, value] of Object.entries(weights[group])) {
    if (!(Object.hasOwn(data.weights[group], key)) && !(data.canonicalRevision && group === 'skater' && key === 'plus_minus')) throw Error(`Unsupported category: ${key}`);
...
const rows = data.players.map((p, i) => {
  const unavailableCategories = p.canonicalRates ? Object.entries(weights[p.isGoalie ? 'goalie' : 'skater'])
    .filter(([key, weight]) => weight !== 0 && !Number.isFinite(p.stats[key]) &&
      !(p.games === 0 && p.canonicalCounts?.[key] === 0))
    .map(([key]) => key) : [];
  return {...p, sortId:i+1, ...(p.canonicalRates ? {unavailableCategories} : {})};
});
const result = [];
for (const goalie of [false, true]) {
  const cohort = rows.filter(p=>p.isGoalie===goalie && (!p.forecastStatus || p.forecastStatus==='projected') && p.games!==null && !p.unavailableCategories?.length);
