...
from local_access_denial import permission_denial

VERSION = 'citrus-local-model-publication-proof-v2'
...
CODE = ['scripts/local_model_publication_e2e_v2.py', 'scripts/proof/local_access_denial.py',
        'scripts/proof/run_local_model_publication_v4.mjs',
        'scripts/local_model_publication_e2e.py', 'scripts/proof/read_model_publication.ts',
        ...
        for role in ('anon', 'authenticated'):
            ...
            for table in TABLES:
                ...
                try: restricted.select_exact(table, select=column, limit=1)
                    ...
                    denials.append(permission_denial(error, role=role, table=table, operation='select_exact'))
                else: raise ValueError('Ordinary role read private evidence')
                try: restricted.insert(table, [{}])
                except RuntimeError as error:
                    denials.append(permission_denial(error, role=role, table=table, operation='insert'))
                else: raise ValueError('Ordinary role wrote evidence')
            phases.append({'phase': role, 'all_evidence_reads_writes_denied': True, 'denials': denials})
