    ...
    <div className="space-y-4">
      <Card>
        ...
        <CardContent className="space-y-6">
          ...
          {/* Component 2: Live Player News Feed */}
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-semibold flex items-center gap-2">
            ...
            <div className="max-h-48 overflow-y-auto space-y-2">
              {newsLoading ? (
                ...
                newsItems.slice(0, 5).map((item) => {
                  const statusColor = getStatusColor(item.roster_status);
                  const isInjured = item.roster_status !== 'ACT' && item.is_ir_eligible;
                  const nextManUp = nextManUpMap.get(item.player_id);
                  
                  return (
                    <div
                      key={item.player_id}
                      className="p-2 rounded-lg border-2 cursor-pointer hover:shadow-sm transition-all"
                      style={{ borderColor: `${statusColor}40` }}
                      onClick={() => handlePlayerClick(item.player_id)}
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex-1 min-w-0">
                          ...
                          {isInjured && nextManUp && (
                            <div className="mt-1 p-1.5 rounded bg-citrus-sage/10 border border-citrus-sage/30">
                              <div className="text-[10px] font-semibold text-citrus-sage mb-0.5">
                                Next Man Up
                              </div>
                              <div className="text-[10px] text-muted-foreground truncate">
                                {nextManUp.full_name} ({nextManUp.team}) • {nextManUp.gamesThisWeek} games
                              </div>
                              <Button
                                size="sm"
                                variant="outline"
                                ...
                                onClick={(e) => {
                                  e.stopPropagation();
                                  navigate(`/waiver-wire?player=${nextManUp.id}`);
                                }}
                              >
                                Add
                              </Button>
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          >
                            {getStatusLabel(item.roster_status)}
                          </Badge>
                        </div>
                      </div>
          ...
          {/* Component 3: Gap Analysis (Projected Points) */}
          <div className="space-y-2">
            <h3 className="text-sm font-semibold flex items-center gap-2">
            ...
            <div className="space-y-3">
              {depths.map((depth) => {
                const progressWidth = depth.strength === 'Elite' ? 100 : depth.strength === 'Good' ? 75 : depth.strength === 'Average' ? 50 : 25;
                return (
                  <div
                    key={depth.position}
                    className="space-y-1 cursor-pointer hover:opacity-80 transition-opacity"
                    onClick={() => depth.strength === 'Weak' && handleClickToTrade(depth.position)}
                  >
                    <div className="flex items-center justify-between text-xs">
                      <div className="flex items-center gap-2">
                        ...
                        {depth.strength === 'Weak' && (
                          <Badge className="text-[10px] px-1 bg-citrus-orange/20 text-citrus-orange border-citrus-orange/50">
                            Click to Trade
                          </Badge>
                        )}
                      </div>
                      <div className="flex items-center gap-2">
                        <span className="text-xs font-semibold">
                          {depth.projectedPoints.toFixed(1)} pts
                        </span>
                        {leagueAverages.has(depth.position) && (
                          <span className="text-xs text-muted-foreground">
                            (avg: {leagueAverages.get(depth.position)!.toFixed(1)})
                          </span>
                        )}
                        <span
                          className="text-xs font-bold"
                          style={{
                            color: depth.strength === 'Elite' ? '#459345' :
                                   depth.strength === 'Good' ? '#F9A436' :
                                   depth.strength === 'Average' ? '#F9E076' :
                                   '#FF6F80'
                          }}
                        >
                          {depth.grade}
