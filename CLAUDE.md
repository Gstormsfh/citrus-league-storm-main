...
## Hybrid: Node.js main app + persistent Node draft worker on Cloud Run

Per **ADR-001** (`docs/adr/ADR-001-elixir-phoenix-draft-engine.md`), the live draft engine is built as a **persistent Node.js service running on Cloud Run**, alongside the existing Node server. The service holds per-draft state in memory, communicates with browser clients via WebSocket, and writes durably to the existing Postgres via the Phase 2 RPCs (`submit_pick_v2`, etc.). The rest of the application — leagues, rosters, matchups, scoring dashboards, AI Assistant, public pages — continues on the existing Node.js / Next.js / Supabase stack unchanged.

The driver was the Performance Mandate above: the Phase 0–4 architecture used Supabase Edge Functions as the autopick host, which is the **wrong runtime model** for live multiplayer state. Edge Functions are ephemeral stateless invocations; live drafts demand persistent stateful processes. World-class real-time platforms (Discord, Sleeper, Figma, Yahoo, ESPN) all use persistent stateful server processes. The Edge Function choice was correct for early correctness verification but wrong for the live experience layer Phase 4.5+ is building.

### What runs where

- **Node.js / Next.js / Supabase main app** (existing, unchanged): apps/web (React SPA), server (Hono API), packages/shared, data-pipeline (Python NHL ingest), supabase/migrations, supabase/functions/* except draft-autopick. All non-draft features. All durability storage (Postgres event log, projection tables, RLS).
- **Persistent Node draft worker on Cloud Run** (new, Phase 4.5+): the live draft engine. One Node process holding `DraftRoom` instances in memory — one per active draft. WebSocket transport for picks, broadcasts, timer ticks. Reads the event log via existing Postgres clients on startup; writes via the existing `submit_pick_v2` RPC. Same TypeScript, same npm ecosystem, same observability surface as the main Node server.
- **Edge Function `draft-autopick`** (existing, role changes): demoted from hot-path autopick worker to **cron-driven disaster-recovery safety net only**. The pgmq sweep + keep-alive cron continue to run; if the persistent worker is unavailable mid-draft (deploy, crash, network partition), the Edge Function path still catches missed deadlines and commits picks. Slowly but correctly. No code is deleted; the wiring changes so the worker is the primary path.

### Working in this codebase

- **Sessions touching the draft worker** (anything under the new persistent worker's directory once it lands; or supabase/migrations / server/ touching `submit_pick_v2`, `draft_events`, `draft_picks_v2`, `pgmq` queues, or `draft-autopick`) use the existing TypeScript / Node patterns. The integration boundary is the existing Postgres RPC surface (which doesn't change without an ADR) plus the new WebSocket message protocol (chunk 11g.2's deliverable).
...
- **Sessions touching shared utilities** (`packages/shared/`) need to consider whether the change affects the cross-runtime contract that the persistent worker depends on (`computePickPayloadHash`, `AUTOPICK_NAMESPACE_UUID`, `ScoringSettings`). Same package, same TypeScript, but the worker's deployment is independent of the main server, so changes here require coordinated deploys.

### See also

- `docs/adr/ADR-001-elixir-phoenix-draft-engine.md` — full ADR with research, the Decision History, alternatives (Elixir/Phoenix, Node-in-Edge-Functions optimization), and consequences.
- `docs/PHASE_4_5_PLAN.md` — chunk-by-chunk plan for the persistent Node worker build-out (chunks 11g.1 through 11g.9).
- `docs/REGISTRY.md` — project-wide known-issues registry. KI-008 (architectural pivot from Edge Functions), KI-009 (Edge Functions retained as cron-driven safety net), KI-010 (Tier 1 perf optimizations baked into design from the start) live here.
