...
VERSION = 'citrus-verified-movement-feature-reuse-v2'
...
class Closure:
    ...
    def safe(self, name):
        path = Path(name)
        if '..' in path.parts:
            raise ValueError('Traversal forbidden in absolute and relative inputs')
...
class VerifiedFeatures:
    ...
    def verify(self):
        self.closure.verify()
        validate_configuration(self.schema, self.config)
        validate_memberships(self.folds, self.schema)
        if content_seal(self.folds, self.groups, self.schema, self.config) != self.content_seal:
            raise ValueError('In-memory certified feature/cohort drift')


def content_seal(folds, groups, schema, config):
    return fingerprint({'schema': schema, 'config': config,
        'folds': {f: {s: {k: v for k, v in p.items() if k != 'rows'} for s, p in parts.items()} for f, parts in folds.items()},
        'groups': groups})
...
def load(root):
    ...
    return VerifiedFeatures(folds, grouped, schema, config, cache_key, certification, closure,
                            content_seal(folds, grouped, schema, config))
