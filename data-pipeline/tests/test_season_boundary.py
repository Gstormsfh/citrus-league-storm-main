...
So SEASON_START_DATES is consulted first and the calendar rule is kept for
...
def test_product_path_season(d, expected, why):
    assert current_nhl_season(d) == expected, why
...
def test_season_starts_are_sane():
    ...
    for season, start in SEASON_START_DATES.items():
...
def test_every_season_start_resolves_to_itself():
    for season, start in SEASON_START_DATES.items():
        assert current_nhl_season(start) == season, f"{start} must resolve to {season}"
...
def test_september_opener_does_not_start_early():
    ...
    for season, start in SEASON_START_DATES.items():
        if start.month == 9:
            ...
            assert current_nhl_season(day_before) == season - 1, (
