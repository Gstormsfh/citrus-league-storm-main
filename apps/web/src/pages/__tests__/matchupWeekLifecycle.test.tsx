...
function Harness({ week }: { week: number }) {
  ...
  const stable = useRef({
    ...
    MatchupService: {
      updateMatchupScores: async () => ({ error: null }),
      getUserMatchup: async (_l: string, _u: string, w: number) => ({ matchup: fixture(w).data.matchup }),
      getMatchupData: (_l: string, _u: string, w: number) => { calls.push(w); return replies.get(w)!.promise; },
    },
    matchupApi: { ensureRosters: async () => ({}), getFrozenRosterBatch: async (id: string) => frozen && id === 'matchup-1' ? frozen.promise : { data: [] } },
  ...
  const scope = new Proxy({ ...stable.current, urlLeagueId: 'league', urlWeekId: String(week),
...
beforeEach(() => { vi.useFakeTimers(); calls = []; replies = new Map([1,2,3].map(w => [w, deferred()])); frozen = null; });
