    ...
    const load = async () => {
      try {
        const [bracketRes, picksRes, h2hRes] = await Promise.all([
          fetch('/api/nhl-playoffs/bracket').then(r => r.json()),
          ...
          fetch('/api/nhl-playoffs/h2h').then(r => r.json()).catch(() => null),
        ]);
      ...
      <div className="max-w-5xl mx-auto">
        ...
        {availableValues.length > 0 && (
          <Card className="mb-4 bg-[#1A2A20] border-0 ring-1 ring-pastel-orange/30 rounded-2xl p-3 shadow-[0_8px_24px_-12px_rgba(255,168,87,0.2)]">
            <div className="text-[10px] font-display font-bold uppercase text-white/70 mb-2">Available Confidence Points</div>
                    ...
                    return (
                      <div key={s.series_id} className={cn(
                        ...
                        {h2hMap[s.bracket_slot] && h2hMap[s.bracket_slot].games > 0 && high && low && (() => {
                          ...
                          return (
                            <div className="flex items-center justify-center gap-1.5 text-[10px] text-white/55 pt-1 border-t border-white/10">
                              ...
                              <span className="text-white/70">({h2hMap[s.bracket_slot].games} games)</span>
                            </div>
                          );
                        })()}
                        ...
                        {pick?.picked_team_id && !locked && (
                          <div>
                            <div className="text-[10px] font-display text-white/70 mb-1">Confidence:</div>
        ...
        {series.length === 0 && (
          <Card className="p-8 text-center">
            <AlertTriangle className="h-8 w-8 text-white/70 mx-auto mb-2" aria-hidden="true" />
        ...
        <Card className="mt-6 bg-[#1A2A20] border-0 ring-1 ring-pastel-sage/30 rounded-2xl px-4 py-3 shadow-[0_8px_24px_-12px_rgba(166,211,160,0.15)]">
          <div className="text-[10px] font-display font-bold uppercase text-white/70 mb-1">How Confidence Pools Work</div>
