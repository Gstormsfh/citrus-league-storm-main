      ...
      if (!exists) {
        ...
        const { data: league, error: leagueError } = await supabase
          ...
          .insert({
            id: DEMO_LEAGUE_ID,
            name: 'Demo League',
            // leagues.commissioner_id is NOT NULL. This wrote null with the comment
            // "No owner - system league", so the insert was always rejected and the
            // demo league could never be seeded (2026-08-12, ledger 342). The block
            // directly above re-verifies auth precisely because a write is coming --
            // use the user it just verified.
            commissioner_id: verifyUser.id,
            roster_size: 21,
            draft_rounds: 21,
