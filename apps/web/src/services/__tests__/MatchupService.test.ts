...
// =============================================================================
// transformToMatchupPlayerWithGames — headshot passthrough (audit M4)
// =============================================================================

describe('MatchupService.transformToMatchupPlayerWithGames — image passthrough', () => {
  const hockeyPlayer = (over: Record<string, unknown> = {}) => ({
    id: 8478402,
    name: 'Connor McDavid',
    position: 'C',
    number: 97,
    ...
    stats: { goals: 10, assists: 20, shots: 80, blockedShots: 5, gamesPlayed: 20, xGoals: 8.5, powerPlayPoints: 9 },
    ...over,
  });
  const weekStart = new Date('2025-01-12T00:00:00');
  const weekEnd = new Date('2025-01-18T00:00:00');
  const transform = (p: Record<string, unknown>) =>
    MatchupService.transformToMatchupPlayerWithGames(p as any, true, weekStart, weekEnd, 'America/Denver', []);

  it('carries the headshot URL from HockeyPlayer.image to MatchupPlayer.image', () => {
    // Every producer already had the URL (Player.headshot_url →
    // transformToHockeyPlayer sets image); this transform used to drop it,
    // which is why no matchup row could draw a face.
    const out = transform(hockeyPlayer());
    expect(out.image).toBe('https://assets.nhle.com/mugs/nhl/20252026/EDM/8478402.png');
    expect(out.name).toBe('Connor McDavid');
    expect(out.team).toBe('EDM');
  });

  it('normalises a missing or empty image to undefined, not an empty string', () => {
    expect(transform(hockeyPlayer({ image: undefined })).image).toBeUndefined();
    expect(transform(hockeyPlayer({ image: '' })).image).toBeUndefined();
  });

  it('keeps the image on the goalie branch too', () => {
    const out = transform(
      hockeyPlayer({
        id: 8476945,
        ...
        image: 'https://assets.nhle.com/mugs/nhl/20252026/WPG/8476945.png',
        stats: { wins: 30, saves: 1200, gamesPlayed: 40 },
      }),
    );
    expect(out.isGoalie).toBe(true);
    expect(out.image).toBe('https://assets.nhle.com/mugs/nhl/20252026/WPG/8476945.png');
  });
});
