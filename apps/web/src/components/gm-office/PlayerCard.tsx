        ...
        <div className="absolute inset-0 bg-gradient-to-t from-citrus-forest/90 via-citrus-forest/40 to-transparent flex flex-col justify-end p-3">
          <Badge 
            className={`absolute top-2 right-2 font-varsity font-black text-xs border-2 shadow-patch ${
              player.starter 
                ? "bg-citrus-sage border-citrus-forest text-[#E8EED9]" 
                : "bg-[#E8EED9]/50 backdrop-blur-sm/80 border-citrus-sage text-citrus-forest"
            }`}
          >
            {getAbbreviation(player.position)}
          </Badge>
          <h3 className="text-[#E8EED9] font-varsity font-bold uppercase tracking-tight">{player.name}</h3>
          <div className="flex justify-between items-center">
            <span className="text-[#E8EED9]/90 text-xs font-display">{player.team}</span>
            <span className="text-[#E8EED9]/90 text-xs font-mono">#{player.number}</span>
          </div>
        </div>
      </div>
      <div className="p-3 bg-gradient-to-br from-citrus-cream to-citrus-sage/10 border-t-4 border-citrus-sage/40">
        <div className="grid grid-cols-3 gap-2 text-xs">
          <div className="text-center p-2 bg-[#E8EED9]/50 backdrop-blur-sm/60 rounded-lg border-2 border-citrus-sage/30">
          ...
          <div className="text-center p-2 bg-[#E8EED9]/50 backdrop-blur-sm/60 rounded-lg border-2 border-citrus-sage/30">
            ...
            <div className="font-varsity font-black text-lg text-citrus-forest">{isGoalie ? (Number(player.stats.gaa ?? 0).toFixed(2)) : player.stats.assists}</div>
          </div>
          <div className="text-center p-2 bg-[#E8EED9]/50 backdrop-blur-sm/60 rounded-lg border-2 border-citrus-sage/30">
            <div className="text-citrus-charcoal font-display font-semibold">{isGoalie ? "SV%" : "PTS"}</div>
            <div className="font-varsity font-black text-lg text-citrus-forest">{isGoalie ? (player.stats.savePct ? (Number(player.stats.savePct) * 100).toFixed(3) : '0.000') + '%' : player.stats.points}</div>
          </div>
        </div>
      </div>
    </Card>
