...
function WebDraftKitDownloads() {
  ...
  async function checkout(){
    ...
    try {
      const result=await apiClient.post<{url:string}>('/api/draft-kit/checkout/session',{}, {retries:0});
  ...
  async function download(format:string){
    ...
    try {
      ...
      setMessage(format==='desk'?'Offline desk downloaded. Open the HTML for manual pick tracking and save your progress. On Citrus, your purchased Draft Desk opens automatically in the draft room. It uses published projections, which may differ from this dated download.':format==='csv'?'CSV downloaded. Blank stats mean unavailable or not applicable, not zero.':'Download ready. Save your copy before editing or printing.');
