...
export function DraftDeskPanel({ live, scoring, scoringReady, initialFile, cloud, onReturnToDraft }: {
  ...
  const remaining = !!cloud || file?.kit.projectionBasis === 'remaining_season';
    ...
    <div className="citrus-desk-body space-y-3">
      ...
      {file && <>
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          {cloud && <p role="status" className="text-xs text-[#526759]">{cloud.status === 'Saved' ? mobile ? 'Saved to your account' : 'Notes and targets saved to your account.' : cloud.status}</p>}
        </div>
        ...
        {unavailable ? <p className="p-4 text-sm">Availability is hidden until the draft connection can be verified. Your notes are retained.</p> : <>
          ...
          <p className="citrus-desk-count text-xs text-[#526759]"><strong>{visible.length}</strong> shown · {file.kit.players.length - taken} available · {remainingTargets} targets left{mobile && <span className="block">{remaining ? 'Remaining-season projections' : 'Season projections'} · {cloud ? 'Scored for your league' : 'Scored for your kit'}</span>}</p>
