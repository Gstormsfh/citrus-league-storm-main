...
async function capture(tabId,previous){
  const replies=await chrome.scripting.executeScript({target:{tabId,frameIds:[0]},world:'ISOLATED',func:captureEspnDraft,args:[previous]});
...
chrome.runtime.onMessage.addListener((message,sender,respond)=>{
  if(sender.id!==chrome.runtime.id||sender.url!==chrome.runtime.getURL('popup.html')||message?.type!=='START'||!Number.isInteger(message.tabId))return;
  ...
  (async()=>{
    ...
    const snapshot=await capture(message.tabId,null);
    ...
    return {ok:true};
  })().then(respond).catch(()=>respond(failure('Could not connect. Reopen the draft’s Rules tab and retry.'))).finally(()=>starting.delete(message.tabId));
  return true;
});
